    ...
    <>
      <nav style={{
        ...
        <div className="nav-links" style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          <a href="https://github.com/sahil-develop" target="_blank" rel="noopener"
            style={{
              ...
              fontFamily: "var(--font-mono)", textDecoration: "none", transition: "all 0.2s",
            }}
            onMouseEnter={e => {
        ...
        <button className="nav-hamburger" onClick={() => setMenuOpen(m => !m)} aria-label="Menu">
          <span style={{ display: "block", width: "22px", height: "2px", background: "var(--accent)", transition: "transform 0.3s", transform: menuOpen ? "rotate(45deg) translate(5px, 5px)" : "none" }} />
          <span style={{ display: "block", width: "22px", height: "2px", background: "var(--accent)", transition: "opacity 0.3s", opacity: menuOpen ? 0 : 1 }} />
          <span style={{ display: "block", width: "22px", height: "2px", background: "var(--accent)", transition: "transform 0.3s", transform: menuOpen ? "rotate(-45deg) translate(5px, -5px)" : "none" }} />
        </button>
      </nav>
      ...
      {menuOpen && (
        <div className="nav-mobile-menu">
          {links.map(l => (
            <button key={l} onClick={() => scroll(l)}
              style={{ color: active === l ? "var(--accent)" : undefined }}
            >{l}</button>
          ))}
          <a href="https://github.com/sahil-develop" target="_blank" rel="noopener">GitHub ↗</a>
        </div>
      )}
    </>
  );
